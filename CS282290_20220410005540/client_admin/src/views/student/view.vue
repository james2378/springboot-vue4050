<template>
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','tutor_no') || $check_field('add','tutor_no') || $check_field('set','tutor_no')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="辅导员工号" prop="tutor_no">
						<div v-if="user_group !== '管理员'">
							{{ get_user_session_tutor_no(form_sub['tutor_no']) }}
							<!--<el-input id="business_name" v-model="form['tutor_no']" placeholder="请输入辅导员工号"-->
							<!--v-if="user_group === '管理员' || (form['student_id'] && $check_field('set','tutor_no')) || (!form['student_id'] && $check_field('add','tutor_no'))" :disabled="disabledObj['tutor_no_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','tutor_no')">{{form['tutor_no']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['student_id'] && $check_field('set','tutor_no')) || (!form['student_id'] && $check_field('add','tutor_no'))" id="tutor_no" v-model="form_sub['tutor_no']" :disabled="disabledObj['tutor_no_isDisabled']">
								<el-option v-for="o in list_user_tutor_no" :key="o['username']" :label="o['nickname'] + '-' + o['username']"
										   :value="o['user_id']">
								</el-option>
							</el-select>
							<el-select v-else-if="$check_field('get','tutor_no')" id="tutor_no" v-model="form_sub['tutor_no']" :disabled="true">
								<el-option v-for="o in list_user_tutor_no" :key="o['username']" :label="o['nickname'] + '-' + o['username']"
										   :value="o['user_id']">
								</el-option>
							</el-select>
						</div>
						<el-select v-else id="tutor_no" v-model="form_sub['tutor_no']">
							<el-option v-for="o in list_user_tutor_no" :key="o['username']" :label="o['nickname'] + '-' + o['username']"
									   :value="o['user_id']">
							</el-option>
						</el-select>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','counselor_name') || $check_field('add','counselor_name') || $check_field('set','counselor_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="辅导员姓名" prop="counselor_name">
					<el-input id="counselor_name" v-model="form_sub['counselor_name']" placeholder="请输入辅导员姓名"
							  v-if="user_group === '管理员' || (form_sub['student_id'] && $check_field('set','counselor_name')) || (!form_sub['student_id'] && $check_field('add','counselor_name'))"></el-input>
					<div v-else-if="$check_field('get','counselor_name')">{{form_sub['counselor_name']}}</div>
				</el-form-item>
			</el-col>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','student_name') || $check_field('add','student_name') || $check_field('set','student_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="学生姓名" prop="student_name">
					<el-input id="student_name" v-model="form_sub['student_name']" placeholder="请输入学生姓名"
							  v-if="user_group === '管理员' || (form_sub['student_id'] && $check_field('set','student_name')) || (!form_sub['student_id'] && $check_field('add','student_name'))"></el-input>
	...
	export default {
		...
		data() {
			return {
				...
				disabledObj:{
					"counselor_name_isDisabled": false,
		...
		methods: {
			 ...
			 * 获取辅导员用户组
			 */
			async get_group_user_tutor_no() {
				this.form_sub["tutor_no"] = this.user.user_id;
				var json = await this.$get("~/api/user_group/get_obj?name=辅导员");
				if(json.result && json.result.obj){
					this.group_user_tutor_no = json.result.obj;
				}
				else if(json.error){
					console.error(json.error);
				}
			},
			get_user_session_tutor_no(id){
				var _this = this;
				var user_id = {"user_id":id}
				var url = "~/api/"+_this.group_user_tutor_no.source_table+"/get_obj?"
					...
					if (res.result && res.result.obj) {
						...
						for (var i=0;i<arr.length;i++){
							for (var j=0;j<arrForm.length;j++){
								if (arr[i]===arrForm[j]){
									if (arr[i]!=="tutor_no") {
										_this.form_sub[arrForm[j]] = res.result.obj[arr[i]]
