		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','tutor_no') || $check_field('add','tutor_no') || $check_field('set','tutor_no')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="辅导员工号" prop="tutor_no">
					<el-input id="tutor_no" v-model="form_sub['tutor_no']" placeholder="请输入辅导员工号"
							  v-if="user_group === '管理员' || (form_sub['instructor_id'] && $check_field('set','tutor_no')) || (!form_sub['instructor_id'] && $check_field('add','tutor_no'))"></el-input>
					<div v-else-if="$check_field('get','tutor_no')">{{form_sub['tutor_no']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','counselor_name') || $check_field('add','counselor_name') || $check_field('set','counselor_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="辅导员姓名" prop="counselor_name">
					<el-input id="counselor_name" v-model="form_sub['counselor_name']" placeholder="请输入辅导员姓名"
							  v-if="user_group === '管理员' || (form_sub['instructor_id'] && $check_field('set','counselor_name')) || (!form_sub['instructor_id'] && $check_field('add','counselor_name'))"></el-input>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["instructor_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?instructor_id=" + form_sub["instructor_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/instructor/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/instructor/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/instructor/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/instructor/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/instructor/view','get');
