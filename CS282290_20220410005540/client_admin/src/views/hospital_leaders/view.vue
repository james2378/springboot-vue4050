<template>
	<el-main class="bg edit_wrap">
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','institute_leader_no') || $check_field('add','institute_leader_no') || $check_field('set','institute_leader_no')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="院领导编号" prop="institute_leader_no">
					<el-input id="institute_leader_no" v-model="form_sub['institute_leader_no']" placeholder="请输入院领导编号"
							  v-if="user_group === '管理员' || (form_sub['hospital_leaders_id'] && $check_field('set','institute_leader_no')) || (!form_sub['hospital_leaders_id'] && $check_field('add','institute_leader_no'))"></el-input>
					<div v-else-if="$check_field('get','institute_leader_no')">{{form_sub['institute_leader_no']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','name_of_institute_leader') || $check_field('add','name_of_institute_leader') || $check_field('set','name_of_institute_leader')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="院领导姓名" prop="name_of_institute_leader">
					<el-input id="name_of_institute_leader" v-model="form_sub['name_of_institute_leader']" placeholder="请输入院领导姓名"
							  v-if="user_group === '管理员' || (form_sub['hospital_leaders_id'] && $check_field('set','name_of_institute_leader')) || (!form_sub['hospital_leaders_id'] && $check_field('add','name_of_institute_leader'))"></el-input>
					<div v-else-if="$check_field('get','name_of_institute_leader')">{{form_sub['name_of_institute_leader']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','post') || $check_field('add','post') || $check_field('set','post')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="职务" prop="post">
					<el-input id="post" v-model="form_sub['post']" placeholder="请输入职务"
							  v-if="user_group === '管理员' || (form_sub['hospital_leaders_id'] && $check_field('set','post')) || (!form_sub['hospital_leaders_id'] && $check_field('add','post'))"></el-input>
					<div v-else-if="$check_field('get','post')">{{form_sub['post']}}</div>
				</el-form-item>
			</el-col>
	...
	export default {
		...
		data() {
			return {
				...
				disabledObj:{
					"institute_leader_no_isDisabled": false,
					"name_of_institute_leader_isDisabled": false,
					"post_isDisabled": false,
				},

				form_sub: {
					"institute_leader_no":'', // 院领导编号
					"name_of_institute_leader":'', // 院领导姓名
					"post":'', // 职务
				    "user_id": 0,
					"hospital_leaders_id": 0 // ID
				},
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["hospital_leaders_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?hospital_leaders_id=" + form_sub["hospital_leaders_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/hospital_leaders/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/hospital_leaders/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/hospital_leaders/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/hospital_leaders/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/hospital_leaders/view','get');
