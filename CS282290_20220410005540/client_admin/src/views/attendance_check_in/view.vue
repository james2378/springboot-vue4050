		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			<el-col v-if="user_group === '管理员' || $check_field('get','teacher_job_number') || $check_field('add','teacher_job_number') || $check_field('set','teacher_job_number')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="教师工号" prop="teacher_job_number">
						<el-select v-if="user_group === '管理员' || (form['attendance_check_in_id'] && $check_field('set','teacher_job_number')) || (!form['attendance_check_in_id'] && $check_field('add','teacher_job_number'))" id="teacher_job_number" v-model="form['teacher_job_number']" :disabled="disabledObj['teacher_job_number_isDisabled']">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','teacher_name') || $check_field('add','teacher_name') || $check_field('set','teacher_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="教师姓名" prop="teacher_name">
					<el-input id="teacher_name" v-model="form['teacher_name']" placeholder="请输入教师姓名"
							  v-if="user_group === '管理员' || (form['attendance_check_in_id'] && $check_field('set','teacher_name')) || (!form['attendance_check_in_id'] && $check_field('add','teacher_name'))" :disabled="disabledObj['teacher_name_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','attendance_name') || $check_field('add','attendance_name') || $check_field('set','attendance_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="考勤名称" prop="attendance_name">
					<el-input id="attendance_name" v-model="form['attendance_name']" placeholder="请输入考勤名称"
							  v-if="user_group === '管理员' || (form['attendance_check_in_id'] && $check_field('set','attendance_name')) || (!form['attendance_check_in_id'] && $check_field('add','attendance_name'))" :disabled="disabledObj['attendance_name_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','student_number') || $check_field('add','student_number') || $check_field('set','student_number')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="学号" prop="student_number">
						<div v-if="user_group !== '管理员'">
							...
							<!--v-if="user_group === '管理员' || (form['attendance_check_in_id'] && $check_field('set','student_number')) || (!form['attendance_check_in_id'] && $check_field('add','student_number'))" :disabled="disabledObj['student_number_isDisabled']"></el-input>-->
							<!--<div v-else-if="$check_field('get','student_number')">{{form['student_number']}}</div>-->
							<el-select v-if="user_group === '管理员' || (form['attendance_check_in_id'] && $check_field('set','student_number')) || (!form['attendance_check_in_id'] && $check_field('add','student_number'))" id="student_number" v-model="form['student_number']" :disabled="disabledObj['student_number_isDisabled']">
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','student_name') || $check_field('add','student_name') || $check_field('set','student_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="学生姓名" prop="student_name">
					<el-input id="student_name" v-model="form['student_name']" placeholder="请输入学生姓名"
							  v-if="user_group === '管理员' || (form['attendance_check_in_id'] && $check_field('set','student_name')) || (!form['attendance_check_in_id'] && $check_field('add','student_name'))" :disabled="disabledObj['student_name_isDisabled']"></el-input>
			...
			<el-col v-if="user_group === '管理员' || $check_field('get','class_name') || $check_field('add','class_name') || $check_field('set','class_name')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="班级名称" prop="class_name">
					<el-input id="class_name" v-model="form['class_name']" placeholder="请输入班级名称"
							  v-if="user_group === '管理员' || (form['attendance_check_in_id'] && $check_field('set','class_name')) || (!form['attendance_check_in_id'] && $check_field('add','class_name'))" :disabled="disabledObj['class_name_isDisabled']"></el-input>
					<div v-else-if="$check_field('get','class_name')">{{form['class_name']}}</div>
				</el-form-item>
			</el-col>
			<el-col v-if="user_group === '管理员' || $check_field('get','attendance_status') || $check_field('add','attendance_status') || $check_field('set','attendance_status')" :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				<el-form-item label="考勤状态" prop="attendance_status">
					<el-select id="attendance_status" v-model="form['attendance_status']"
						v-if="user_group === '管理员' || (form['attendance_check_in_id'] && $check_field('set','attendance_status')) || (!form['attendance_check_in_id'] && $check_field('add','attendance_status'))">
...
<script>
	...
	export default {
		...
		data() {
			return {
				field: "attendance_check_in_id",
				url_add: "~/api/attendance_check_in/add?",
				url_set: "~/api/attendance_check_in/set?",
				url_get_obj: "~/api/attendance_check_in/get_obj?",
				url_upload: "~/api/attendance_check_in/upload?",

				query: {
					"attendance_check_in_id": 0,
				},
				...
				//考勤状态选项列表
				list_attendance_status: ['正常','请假','迟到','早退'],
			}
		},
		methods: {
			/**
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/attendance_check_in/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/attendance_check_in/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/attendance_check_in/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/attendance_check_in/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/attendance_check_in/view','get');
